...
:::note alert
GitHubやGoogleアカウントでログインして使っている場合、有料プランを契約していても業務コードやプロンプトがモデル学習に使われる可能性があります。エンタープライズ用途では必ずログイン経路を確認してください。
:::
...
:::note info
Amazon Q Developer Proのサブスクリプションを持っていて、そのAWSアカウント経由でKiroにアクセスしている場合も、サービス改善には利用されないとされています。
:::
...
:::note warn
データの国内保存など保存リージョンに制約がある場合、individual subscriberでは米国東部固定になる点に注意してください。enterprise userであればプロファイルのリージョンを選べます。
:::
